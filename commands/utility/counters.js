...
module.exports = class Counters extends Command {
  ...
  async run(message, args) {
    ...
    let totalTickets = db.fetch(`ticketCount_${message.guild.id}`) || 0;
    ...
    let chOpened = await message.guild.channels.create(`${this.client.language.utility.opened_counter.replace("<stats>", currentTickets.length)}`, {
  ...
  async slashRun(interaction, args) {
    ...
    let currentTickets = db
      .all()
      .filter((i) => i.ID.startsWith("tickets_"));
    let totalTickets = db.fetch(`ticketCount_${interaction.guild.id}`) || 0;
    ...
    let chOpened = await interaction.guild.channels.create(`${this.client.language.utility.opened_counter.replace("<stats>", currentTickets.length)}`, {
      type: config.general.stats_type,
      parent: chCategory,
      permissionOverwrites: [
        {
